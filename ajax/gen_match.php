...
if(isset($_POST['MatchID']) && isset($_POST['id'])) {
	$match = $db->select("`Match`", "Round, EntryID1", "MatchID='$_POST[MatchID]'")[0];
	$round = $match['Round']+1;
	...
	m.Result IS NULL")[0]['COUNT(*)'] == 0) { // all results in from previous round
		if($round >= ceil(log(count($initialEntries), 2))) {// final round
			$lastMatch = $db->select("`Match` as m , Entry as e", "EntryID1, EntryID2, Result",
				...
				e.TournamentID = '$_POST[id]' AND m.Round = $round-1")[0];
			print_r($lastMatch);
...
} elseif(isset($_POST['id'])) { // first round
	$initialEntries = $db->select("Entry", "EntryID", "TournamentID=$_POST[id]");
	$initialEntries = merge($initialEntries, []);
